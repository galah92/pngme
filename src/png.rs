...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let chunks = self.chunks.iter().flat_map(|chunk| chunk.as_bytes());
        self.header().iter().cloned().chain(chunks).collect()
    }
}

#[derive(Debug, thiserror::Error)]
pub enum PngError {
    #[error("could not parse png header")]
    HeaderParseError,
    #[error("mismatch png header")]
    HeaderMismatch,
    #[error("could not parse chunk")]
    ChunkParseError,
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        let header: &[u8; 8] = bytes[..8]
            .try_into()
            .map_err(|_| PngError::HeaderParseError)?;
        if header != &Png::STANDARD_HEADER {
            return Err(PngError::HeaderMismatch);
        }

        let mut chunks = Vec::<Chunk>::new();
        let mut cursor = 8_usize; // because we already parsed the header
        while cursor < bytes.len() {
            let chunk_length = u32::from_be_bytes(
                bytes[cursor..cursor + 4]
                    .try_into()
                    .map_err(|_| PngError::ChunkParseError)?,
            );
            // length value does not include the length, the chunk type and the crc fields
            let chunk_length = (chunk_length + 12) as usize;

            let chunk = &bytes[cursor..cursor + chunk_length];
            let chunk = Chunk::try_from(chunk).map_err(|_| PngError::ChunkParseError)?;
            chunks.push(chunk);
            cursor += chunk_length;
        }
